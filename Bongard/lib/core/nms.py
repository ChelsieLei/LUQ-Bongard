...
def nms_core(pose_coord, heat_score):
    ...
    pose_dist.sqrt_()
    pose_thre = 0.05 * torch.sqrt(pose_area)
    pose_dist = (pose_dist < pose_thre).sum(2)
    nms_pose = pose_dist > 8
...
def pose_nms(heatmap_avg, poses):
    """
    ...
    for i, pose in enumerate(poses):
        if i != scale1_index:
            ...
            pose[:,:,2] = pose[:,:,2]/max_score_scale*max_score*0.9
    ...
    keep_pose_inds = nms_core(pose_coord, heat_score)
    poses = poses[keep_pose_inds]
    heat_score = heat_score[keep_pose_inds]
    
    if len(keep_pose_inds) > 30:
        heat_score, topk_inds = torch.topk(heat_score,30)
