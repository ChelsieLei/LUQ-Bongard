...
class Subspace_transd_Projection(nn.Module):
    ...
    def projection_metric(self, target_features, hyperplanes, common_feat_u):
        eps = 1e-12
        ...
        ## calculate contrastive-like loss
        # mask = (torch.eye(class_size)).view(-1).cuda(non_blocking=True)
        # cts_like_loss = -torch.log(torch.div(torch.exp(similarities/self.temperature).view(-1), torch.sum(torch.exp(similarities/self.temperature).view(-1))))  
        ## only calculate positive class cts loss
        # cts_like_loss = cts_like_loss[0]
        ### calculate all class  loss
        # cts_like_loss = torch.sum(cts_like_loss.mul(mask))/class_size   
        
        return similarities, discriminative_loss, projected_query_dist_inter
